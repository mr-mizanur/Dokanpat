    ...
    <div className="min-h-screen bg-slate-50 text-slate-800">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-white border border-orange-100 p-6 sm:p-8 rounded-3xl shadow-sm">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-orange-50 border border-orange-100 text-orange-600 text-xs font-semibold">
              ...
              <ShieldCheck className="w-3.5 h-3.5 text-orange-600 ml-0.5" />
            </div>
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs sm:text-sm text-slate-500 flex items-center gap-1.5">
                ...
                <span className="text-orange-600 font-medium">@{username}</span>
              </p>
            </div>
          ...
          <div className="flex items-center gap-4 bg-orange-50/40 border border-orange-100 px-5 py-3 rounded-2xl shadow-sm">
            <Link href="/cart" className="relative flex items-center gap-2">
              <ShoppingCart className="w-6 h-6 text-orange-600" />
              {totalItems > 0 && (
                <span className="absolute -top-2 -right-2 w-5 h-5 bg-orange-600 text-white rounded-full text-[10px] font-bold flex items-center justify-center">
                  {totalItems}
                </span>
              )}
            </Link>
            ...
            {totalItems > 0 && (
              <Link href="/checkout" className="ml-2 px-4 py-2 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white text-xs font-semibold rounded-xl transition-all flex items-center gap-1 shadow-md shadow-orange-500/20">
                <span>Checkout</span>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-12 space-y-8">
        <div className="flex items-center justify-between">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-900">Store Products</h2>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-white border border-orange-100 rounded-3xl p-8 space-y-3 shadow-sm">
            ...
            <p className="text-xs text-slate-500">This seller hasn&apos;t published any items to their store yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {products.map((product) => (
              <div key={product._id} className="bg-white border border-orange-100 rounded-2xl overflow-hidden shadow-sm flex flex-col justify-between group hover:border-orange-300 transition-all">
                <div>
                  <div className="relative h-52 w-full bg-slate-100 overflow-hidden">
                    ...
                    />
                    <span className="absolute top-3 right-3 px-3 py-1 bg-white/90 border border-orange-100 backdrop-blur-md rounded-full text-[10px] font-semibold text-orange-600 uppercase tracking-wider">
                      {product.category}
                    </span>
                  </div>
                  ...
                  <div className="p-5 space-y-2">
                    ...
                    <p className="text-xs text-slate-600 line-clamp-2">{product.description || 'No description provided.'}</p>
